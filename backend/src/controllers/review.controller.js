...
const addReview = async (req, res) => {
  try {
    ...
    const hasCompleted = await appointment.Appointment.findOne({
      patientId,
      doctorId,
...
const getDoctorReviews = async (req, res) => {
  try {
    ...
    const reviews = await review.Review.find({ targetType: "doctor", doctorId }).sort({ createdAt: -1 }).limit(50).populate("patientId", "name").lean();
