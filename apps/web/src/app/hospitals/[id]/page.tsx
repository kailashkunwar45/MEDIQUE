...
type HospitalData = {
  hospital: { _id: string; name: string; address: string; contactEmail: string; contactPhone?: string; subscriptionTier: string };
  doctors: Doctor[];
  stats: { avgRating: number; totalReviews: number };
};

type Review = {
  _id: string; rating: number; comment?: string; createdAt: string; patientId?: { name: string };
};
    ...
    <span className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <button
          ...
          onClick={() => onChange?.(s)}
          className={`text-xl ${interactive ? "hover:scale-110 transition-transform cursor-pointer" : "cursor-default"} ${s <= Math.round(value) ? "text-amber-400" : "text-muted-foreground/30"}`}
        >
          ★
        </button>
      ))}
...
export default function HospitalDetailPage() {
  ...
  const loadData = () => {
    ...
    Promise.all([hospitalFetch, reviewsFetch, apptFetch])
      .then(([hospitalRes, reviewsRes, appts]) => {
        if (hospitalRes?.hospital) setData(hospitalRes);
        else setError(hospitalRes?.message || "Hospital not found");
        if (Array.isArray(reviewsRes?.reviews)) setReviews(reviewsRes.reviews);
        if (Array.isArray(appts)) {
          const hasCompleted = appts.some(
            (a: any) => String(a.hospitalId?._id || a.hospitalId) === String(id) && a.status === 'completed'
          );
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-5xl mx-auto px-8 py-8 grid gap-8 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card className="rounded-2xl border-muted shadow-lg">
            ...
            <CardContent>
              {doctors.length === 0 ? (
                <div className="text-sm text-muted-foreground">No doctors listed yet.</div>
              ) : (
                <div className="space-y-3">
                  {doctors.map((d) => {
                    const initials = d.name.split(" ").map((w: string) => w[0]).slice(0, 2).join("").toUpperCase();
                    return (
                      <div key={d._id} className="flex items-center gap-3 rounded-xl border border-muted bg-muted/10 p-3 hover:border-primary/30 transition-colors group">
      ...
      {showBooking && data && (
        <BookingModal
          preHospitalId={hospital._id}
          preHospitalName={hospital.name}
