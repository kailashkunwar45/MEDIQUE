...
import { Users, UserCheck, Building2, ShieldCheck, Clock } from "lucide-react";
           ...
           <Tabs defaultValue="patients" className="w-full space-y-8">
                 ...
                 <div className="bg-white rounded-[24px] border border-slate-200 shadow-[0_15px_50px_rgba(15,23,42,0.05)] overflow-hidden">
                    <table className="w-full text-left">
                       ...
                       <tbody className="divide-y divide-slate-50">
                          {patients.length === 0 ? (<tr><td colSpan={4} className="p-20 text-center text-slate-300 font-black uppercase tracking-[0.2em] text-[10px] italic">No active patient files recorded.</td></tr>) : (patients.map(p => (<tr key={p._id} className="hover:bg-slate-50/50 transition-all cursor-pointer group">
                                   <td className="p-8">
              ...
              <TabsContent value="upcoming" className="space-y-6">
                 <div className="grid gap-6">
                    {upcoming.length === 0 ? (<div className="text-center py-32 bg-white rounded-[32px] border border-dashed border-slate-200 shadow-sm">
                       ...
                       </div>) : (upcoming.map(u => (<Card key={u._id} className="rounded-[24px] border-none shadow-[0_15px_40px_rgba(15,23,42,0.06)] bg-white hover:scale-[1.01] transition-all group overflow-hidden">
              ...
              <TabsContent value="staff" className="space-y-8">
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {doctors.map(d => (<Card key={d._id} className="rounded-[24px] border-none shadow-[0_15px_40px_rgba(15,23,42,0.06)] bg-white p-8 hover:bg-slate-50 transition-all group border-l-4 border-l-[#1E3A8A]">
