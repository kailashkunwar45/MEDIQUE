        ...
        <form onSubmit={handleSubmit}>
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input placeholder="LIC-12345678" className="rounded-xl bg-muted/30" value={formData.certification} onChange={(e) => setFormData({ ...formData, certification: e.target.value })} required/>
            </div>

            <div className="space-y-2">
              <Label>Services Provided (Comma separated)</Label>
              <textarea placeholder="Cardiology, Emergency 24/7, Surgery, Pediatrics..." className="w-full h-32 rounded-xl bg-muted/30 border border-input p-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary/50" value={formData.services} onChange={(e) => setFormData({ ...formData, services: e.target.value })} required/>
            </div>
          </CardContent>
          <CardFooter>
