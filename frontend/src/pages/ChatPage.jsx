...
import { MessageSquare, Search, Send, User as UserIcon, Calendar, Clock, ArrowLeft } from "lucide-react";
...
export default function GlobalChatPage() {
            ...
            socket.on("message", (msg) => {
                ...
                if (msg.appointmentId === activeConvRef.current) {
                    setMessages(prev => {
                        ...
                        const exists = prev.some(m => m.text === msg.text &&
                        ...
                        if (exists) {
                            ...
                            return prev.map(m => (m.text === msg.text && m._id.startsWith('temp-')) ? msg : m);
                        }
                        return [...prev, msg];
                    });
                }
    ...
    const deleteHistory = async () => {
        if (!activeConversationId || !confirm("Are you sure you want to PERMANENTLY delete all messages in this conversation?"))
            return;
        try {
            ...
            setMessages([]);
            setConversations(prev => prev.map(c => c.appointmentId === activeConversationId ? { ...c, lastMessage: undefined } : c));
        }
        catch (e) {
    ...
    const markAsRead = async (appointmentId) => {
        try {
            ...
            setConversations(prev => prev.map(c => c.appointmentId === appointmentId ? { ...c, unreadCount: 0 } : c));
        }
        catch (e) {
    ...
    const sendMessage = async () => {
        if (!inputText.trim() || !activeConversationId || !session)
            return;
    ...
    const filteredConversations = conversations.filter(c => c.partner.name.toLowerCase().includes(searchQuery.toLowerCase()));
    if (loading) {
    ...
    return (<div className="min-h-screen bg-[#F8FAFC] flex flex-col text-slate-900">
        ...
        <div className={`w-full lg:w-96 border-r border-slate-200 bg-white/50 backdrop-blur-sm flex flex-col ${activeConversationId ? "hidden lg:flex" : "flex"}`}>
          <div className="p-6">
            <div className="relative group">
              ...
              <Input placeholder="Search active channels..." className="pl-11 h-12 rounded-[16px] bg-white border-slate-200 focus:border-[#1E3A8A]/50 focus:ring-0 text-xs font-bold text-slate-700 placeholder:text-slate-400 shadow-sm" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
            </div>
          </div>

          <div className="flex-1 overflow-y-auto custom-scrollbar px-3 space-y-2 pb-6">
            {filteredConversations.length === 0 ? (<div className="text-center py-20 px-6 space-y-4">
              ...
              </div>) : (filteredConversations.map((conv) => (<div key={conv.appointmentId} onClick={() => setActiveConversationId(conv.appointmentId)} className={`group relative p-4 rounded-[24px] cursor-pointer transition-all duration-300 ${activeConversationId === conv.appointmentId
                ? "bg-gradient-to-br from-[#1E3A8A] to-[#2563EB] shadow-2xl shadow-blue-500/20"
                : "hover:bg-white/5"}`}>
                  <div className="flex gap-4">
                    <div className="relative">
                      <div className={`w-14 h-14 rounded-[18px] flex items-center justify-center border transition-all duration-300 ${activeConversationId === conv.appointmentId ? "bg-white/20 border-white/30" : "bg-slate-100 border-slate-200"}`}>
                         <UserIcon className={`w-6 h-6 ${activeConversationId === conv.appointmentId ? "text-white" : "text-slate-400"}`}/>
                      </div>
                      {conv.unreadCount > 0 && (<div className="absolute -top-1 -right-1 w-6 h-6 bg-rose-500 border-4 border-slate-900 rounded-full flex items-center justify-center text-[10px] font-black text-white shadow-lg animate-bounce">
                          {conv.unreadCount}
                        </div>)}
                    </div>
                    <div className="flex-1 min-w-0 flex flex-col justify-center">
                      <div className="flex justify-between items-start mb-1">
                        <h3 className={`text-sm font-black truncate tracking-tight transition-colors ${activeConversationId === conv.appointmentId ? "text-white" : "text-slate-900"}`}>
                          {session?.role === 'doctor' ? conv.partner.name : `Dr. ${conv.partner.name}`}
                        </h3>
                        <span className={`text-[9px] font-black uppercase tracking-widest flex-shrink-0 ml-2 ${activeConversationId === conv.appointmentId ? "text-white/80" : "text-slate-400"}`}>
                          {conv.lastMessage ? format(new Date(conv.lastMessage.createdAt), "HH:mm") : format(new Date(conv.date), "MMM d")}
                        </span>
                      </div>
                      <p className={`text-[11px] font-bold truncate leading-relaxed ${activeConversationId === conv.appointmentId ? "text-white/70" : "text-slate-500"}`}>
          ...
          {activeConversationId && activeConv ? (<>
              ...
              <div className="px-8 py-4 bg-white/80 backdrop-blur-md border-b border-slate-200 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-4">
                  <Button variant="ghost" size="icon" className="lg:hidden text-slate-400" onClick={() => setActiveConversationId(null)}>
                ...
                <div className="flex items-center gap-2">
                  <Button variant="ghost" onClick={deleteHistory} className="h-10 rounded-xl px-4 text-[10px] font-black text-rose-500 hover:bg-rose-50 transition-all uppercase tracking-widest flex items-center gap-2 border border-rose-100 shadow-sm">
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8 custom-scrollbar">
                ...
                return (<div key={msg._id || i} className={`flex ${isMe ? "justify-end" : "justify-start"} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                      <div className={`max-w-[70%] group ${isMe ? "items-end" : "items-start"}`}>
                        <div className={`px-6 py-4 rounded-[24px] text-sm font-bold leading-relaxed shadow-md ${isMe
              ...
              <div className="p-8 bg-white/50 border-t border-slate-200 backdrop-blur-sm">
                <div className="max-w-4xl mx-auto flex gap-4">
                  <div className="flex-1 relative">
                    <Input placeholder="Type your message..." className="h-14 rounded-[20px] bg-white border-slate-200 focus:border-[#1E3A8A] focus:ring-0 text-sm font-bold text-slate-900 px-6 shadow-sm" value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={(e) => e.key === "Enter" && sendMessage()}/>
                  </div>
                  <Button className="w-14 h-14 rounded-[20px] bg-[#1E3A8A] hover:bg-[#2563EB] text-white shadow-xl shadow-blue-500/20 transition-all active:scale-95" onClick={sendMessage}>
