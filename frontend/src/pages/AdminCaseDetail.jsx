...
function CaseFileContent() {
    ...
    if (loading)
        return <div className="p-10 text-center font-bold text-slate-400">Opening Case File...</div>;
    if (error)
        return <div className="p-10 text-center text-rose-500 font-bold">{error}</div>;
              ...
              <CardContent>
                 <div className="p-6 rounded-2xl bg-amber-50/30 border border-amber-100 flex flex-col items-center justify-center min-h-[150px]">
                    <div className="flex gap-1 mb-3">
                       {[1, 2, 3, 4, 5].map(s => (<Star key={s} className={`w-6 h-6 ${s <= 4 ? "text-amber-400 fill-amber-400" : "text-slate-200"}`}/>))}
                    </div>
...
export default function CaseFilePage() {
    return (<Suspense fallback={<div className="p-10 text-center font-bold text-slate-400">Loading Suspense...</div>}>
      <CaseFileContent />
    </Suspense>);
}
