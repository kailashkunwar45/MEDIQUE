...
import { ShieldCheck, Stethoscope, Building, PieChart as ChartIcon, AlertCircle, CheckCircle2, XCircle, Zap, Clock, MessageSquare } from "lucide-react";
...
export default function SuperAdminDashboard() {
    ...
    const [rejectingFeeFor, setRejectingFeeFor] = useState(null);
    ...
    useEffect(() => { loadData(); }, []);
    const handleHospitalApproval = async (hospitalId, status) => {
        try {
            ...
            setError("Hospital action failed");
        }
    };
    const handleRemoveHospital = async (hospitalId) => {
        if (!confirm("Are you sure you want to PERMANENTLY remove this hospital?"))
            return;
        try {
            ...
            setError("Remove hospital failed");
        }
    };
    const handleBanHospital = async (hospitalId, currentBanStatus) => {
        const reason = !currentBanStatus ? prompt("Enter ban reason:") : "";
        if (!currentBanStatus && !reason)
            return;
        try {
    ...
    const handleRemoveDoctor = async (doctorId) => {
        ...
        try {
            ...
            setError("Remove doctor failed");
        }
    };
    const handleBanDoctor = async (doctorId, currentBanStatus) => {
        const reason = !currentBanStatus ? prompt("Enter ban reason:") : "";
        if (!currentBanStatus && !reason)
            return;
        try {
    ...
    const handleFeeApproval = async (doctorId, status, reason) => {
    ...
    return (<div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans">
      ...
      <div className="max-w-7xl mx-auto p-8 space-y-10">
        ...
        <Tabs defaultValue="stats" className="space-y-8">
        ...
        ].map((s, i) => (<Card key={i} className="rounded-[20px] border border-slate-100 shadow-[0_10px_30px_rgba(15,23,42,0.05)] bg-white p-6 hover:scale-105 transition-all">
           ...
           <TabsContent value="hospitals" className="space-y-10">
              <div className="space-y-6">
                 ...
                 {pendingHospitals.length === 0 ? (<div className="text-center py-20 bg-white rounded-[20px] border border-dashed border-slate-200 text-slate-400 font-black uppercase tracking-widest text-[10px]">No pending facility registrations.</div>) : (<div className="grid gap-6">
                       {pendingHospitals.map(h => (<Card key={h._id} className="rounded-[20px] border-none shadow-[0_10px_40px_rgba(15,23,42,0.08)] bg-white overflow-hidden hover:scale-[1.01] transition-all">
              ...
              <div className="space-y-6">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {approvedHospitals.map(h => (<Card key={h._id} className={`rounded-[20px] border shadow-sm bg-white p-6 transition-all ${h.isBanned ? 'border-rose-100 bg-rose-50/20' : 'border-slate-100'}`}>
           ...
           <TabsContent value="doctors" className="space-y-10">
              <div className="space-y-6">
                 ...
                 {pendingDoctors.length === 0 ? (<div className="text-center py-20 bg-white rounded-[20px] border border-dashed border-slate-200 text-slate-400 font-black uppercase tracking-widest text-[10px]">All specialist credentials verified.</div>) : (<div className="grid gap-6">
                       {pendingDoctors.map(d => (<Card key={d._id} className="rounded-[20px] border-none shadow-[0_10px_40px_rgba(15,23,42,0.08)] bg-white p-8 flex items-center justify-between flex-wrap gap-6">
                             <div className="flex items-center gap-6">
                                ...
                                <div>
                                   ...
                                   <div className="flex flex-wrap gap-2 mt-4">
                                      {d.hospitalIds?.map((h, idx) => (<span key={idx} className="text-[9px] font-black uppercase bg-slate-50 px-3 py-1 rounded-full text-slate-500 border border-slate-100 tracking-widest">{h.name}</span>))}
                                   </div>
                                </div>
                             </div>
                  ...
                  {viewingDoctor && (<div className="fixed inset-0 bg-black/60 backdrop-blur-md z-[100] flex items-center justify-center p-4 overflow-y-auto">
                          ...
                          <div className="p-8 space-y-6">
                             ...
                             <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                   {viewingDoctor.hospitalIds?.map((h, idx) => (<Badge key={idx} className="bg-indigo-50 text-indigo-600 border-indigo-100 px-3 py-1 rounded-xl font-bold uppercase text-[10px]">{h.name}</Badge>))}
                                </div>
                             </div>
              ...
              <div className="space-y-4">
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {approvedDoctors.map(d => (<Card key={d._id} className={`rounded-3xl border shadow-md bg-white p-6 ${d.isBanned ? 'border-rose-200 bg-rose-50/20' : 'border-slate-100'}`}>
           ...
           <TabsContent value="fees" className="space-y-8">
                ...
                <div className="space-y-4">
                  ...
                  {feeRequests.filter(d => d.pendingFeeUpdate?.status === 'pending').length === 0 ? (<div className="text-center py-12 bg-white rounded-[20px] border border-dashed border-slate-200 text-slate-400 font-black uppercase tracking-widest text-[10px]">No pending fee adjustments.</div>) : (<div className="grid gap-6">
                      {feeRequests.filter(d => d.pendingFeeUpdate?.status === 'pending').map(d => (<Card key={d._id} className="rounded-[20px] border border-amber-100 shadow-[0_10px_40px_rgba(15,23,42,0.06)] bg-white overflow-hidden">
                          <div className="flex items-start justify-between flex-wrap gap-6 p-8">
                            ...
                            <div className="flex flex-col gap-3 min-w-[200px]">
                              {rejectingFeeFor === d._id ? (<div className="space-y-3 animate-in fade-in slide-in-from-top-2 duration-300">
                                  <p className="text-[9px] font-black uppercase tracking-widest text-rose-500">Rejection Reason (Required)</p>
                                  <textarea className="w-full text-xs font-bold border border-rose-200 rounded-[14px] p-3 bg-rose-50 text-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-300 resize-none" placeholder="Provide a clear reason for declining this request..." rows={3} value={rejectReason} onChange={(e) => setRejectReason(e.target.value)}/>
                                  <div className="flex gap-2">
                                    ...
                                    <Button className="flex-1 rounded-[12px] bg-rose-500 hover:bg-rose-600 text-white font-black uppercase text-[9px] tracking-widest h-10 shadow-lg" disabled={!rejectReason.trim()} onClick={() => handleFeeApproval(d._id, 'rejected', rejectReason.trim())}>Confirm Rejection</Button>
                                  </div>
                                </div>) : (<>
                ...
                {/* Decision History */}
                {feeRequests.filter(d => d.pendingFeeUpdate?.status !== 'pending').length > 0 && (<div className="space-y-4">
                ...
                return (<div key={d._id} className={`rounded-[18px] border p-6 flex flex-col md:flex-row gap-6 items-start md:items-center justify-between ${isApproved ? 'bg-emerald-50/50 border-emerald-100' : 'bg-rose-50/50 border-rose-100'}`}>
                            <div className="flex items-center gap-5">
                              <div className={`w-12 h-12 rounded-[14px] flex items-center justify-center text-xl font-black uppercase border ${isApproved ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-rose-100 text-rose-700 border-rose-200'}`}>{d.name[0]}</div>
                              <div>
                                ...
                                <div className="flex items-center gap-4 mt-2">
                                  ...
                                  <span className={`text-[11px] font-black uppercase tracking-widest ${isApproved ? 'text-emerald-600' : 'text-rose-500 line-through'}`}>${d.pendingFeeUpdate?.newFee}</span>
                                </div>
                                {d.pendingFeeUpdate?.reason && (<div className={`mt-2 flex items-start gap-2 p-2 rounded-[10px] border ${isApproved ? 'bg-emerald-100/50 border-emerald-200' : 'bg-rose-100/50 border-rose-200'}`}>
                                    <MessageSquare className={`w-3 h-3 mt-0.5 flex-shrink-0 ${isApproved ? 'text-emerald-600' : 'text-rose-500'}`}/>
                                    <p className={`text-[9px] font-bold italic ${isApproved ? 'text-emerald-700' : 'text-rose-600'}`}>"{d.pendingFeeUpdate.reason}"</p>
                                  </div>)}
                              </div>
                            </div>
                            <div className={`inline-flex items-center gap-2 px-4 py-2 rounded-full font-black uppercase text-[9px] tracking-widest border ${isApproved
