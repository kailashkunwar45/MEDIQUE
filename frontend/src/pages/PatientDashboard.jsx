...
import { CalendarDays, Bell, Trash2 } from "lucide-react";
...
function StatusBadge({ status }) {
    ...
    return (<span className={`text-xs font-semibold px-2 py-0.5 rounded-full border capitalize ${map[status]}`}>
      {status}
    </span>);
}
function PaymentBadge({ status, method, amount }) {
    return (<span className={`text-xs font-semibold px-2 py-0.5 rounded-full border ${status === "paid"
...
export default function PatientDashboard() {
    ...
    const derivedSpecializations = doctors.map(d => d.specialization).filter(Boolean);
    ...
    useEffect(() => {
        if (selectedDoctorId && doctors.length > 0) {
            ...
            if (doc?.hospitalId) {
                const hId = typeof doc.hospitalId === 'object' ? doc.hospitalId._id : doc.hospitalId;
    ...
    const openChat = async (appointmentId) => {
        if (activeChatAppointmentId === appointmentId) {
            ...
            window.__mediqueue_chat_socket?.disconnect?.();
            return;
        }
        setError("");
        setInfo("");
        ...
        socket.on("chatError", (p) => {
            setError(p?.message || "Chat error");
        });
        window.__mediqueue_chat_socket?.disconnect?.();
        window.__mediqueue_chat_socket = socket;
    };
    const sendChat = async () => {
        ...
        if (!text)
            return;
        const socket = window.__mediqueue_chat_socket;
        if (!socket) {
    ...
    const deleteHistory = async (idsToDelete) => {
        const ids = idsToDelete || selectedHistoryIds;
        if (ids.length === 0)
            return;
        if (!confirm(`Are you sure you want to delete ${ids.length} appointment(s) from your history?`))
            return;
        setError("");
        setInfo("");
        setLoading(true);
    ...
    return (<div className="min-h-screen bg-[#F8FAFC] text-slate-900 font-sans">
      ...
      <div className="max-w-7xl mx-auto p-8 space-y-8">
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          ...
          <Card className="rounded-[20px] border border-slate-200 shadow-[0_10px_30px_rgba(15,23,42,0.08)] bg-white overflow-hidden lg:col-span-1">
            ...
            <CardContent className="p-8 space-y-6">
              ...
              <div className="space-y-2">
                ...
                <select className="w-full rounded-[16px] bg-slate-50 border border-slate-200 px-4 py-4 text-sm font-bold text-[#0F172A] outline-none focus:border-accent" value={selectedSpecialization} onChange={handleSpecializationChange} disabled={!session?.accessToken || availableSpecializations.length === 0}>
              ...
              <div className="space-y-2">
                ...
                <select className="w-full rounded-[16px] bg-slate-50 border border-slate-200 px-4 py-4 text-sm font-bold text-[#0F172A] outline-none focus:border-accent" value={selectedDoctorId} onChange={(e) => setSelectedDoctorId(e.target.value)} disabled={!session?.accessToken || filteredDoctors.length === 0}>
                  {filteredDoctors.length === 0 ? (<option>No specialists found</option>) : (filteredDoctors.map((d) => (<option key={d._id} value={d._id}>
              ...
              <div className="space-y-2">
                ...
                <input type="date" value={date} min={new Date().toISOString().slice(0, 10)} onChange={(e) => setDate(e.target.value)} className="w-full rounded-[16px] bg-slate-50 border border-slate-200 px-4 py-4 text-sm font-bold text-[#0F172A] outline-none focus:border-accent" disabled={!session?.accessToken}/>
              </div>

              <div className="space-y-3">
                <Label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">Settlement Protocol</Label>
                <div className="grid grid-cols-2 gap-3">
                  <button type="button" onClick={() => setPaymentMethod("online")} className={`px-4 py-5 text-[10px] font-black uppercase tracking-widest rounded-[16px] border transition-all ${paymentMethod === "online"
            ? "bg-[#1E3A8A] text-white border-[#1E3A8A] shadow-lg gold-glow"
            : "bg-slate-50 text-slate-400 border-slate-200 hover:border-slate-300"}`} disabled={!session?.accessToken}>
                    💳 Instant Settlement<br />
                    <span className="opacity-60 mt-1 block">Pre-Paid</span>
                  </button>
                  <button type="button" onClick={() => setPaymentMethod("pay_later")} className={`px-4 py-5 text-[10px] font-black uppercase tracking-widest rounded-[16px] border transition-all ${paymentMethod === "pay_later"
            ? "bg-[#1E3A8A] text-white border-[#1E3A8A] shadow-lg gold-glow"
            : "bg-slate-50 text-slate-400 border-slate-200 hover:border-slate-300"}`} disabled={!session?.accessToken}>
                    💵 Post-Checkup<br />
          ...
          <Card className="rounded-[20px] border border-slate-200 shadow-[0_10px_30px_rgba(15,23,42,0.08)] bg-white overflow-hidden lg:col-span-2">
            ...
            <CardContent className="p-8">
              {currentView === "active" ? (activeAppointments.length === 0 ? (<div className="text-[10px] font-black uppercase tracking-widest text-slate-500 py-16 text-center opacity-40 italic">System clear. No pending encounters.</div>) : (<div className="space-y-6">
                    {activeAppointments.map((a) => (<div key={a._id} className="rounded-[20px] border border-slate-100 p-6 bg-slate-50/50 hover:bg-white hover:shadow-xl transition-all group border-l-4 border-l-[#1E3A8A]">
                        ...
                        <div className="flex gap-3 items-center relative">
                          {unreadCounts[a._id] > 0 && activeChatAppointmentId !== a._id && (<div className="absolute -top-3 -right-2 flex items-center justify-center bg-rose-500 text-white text-[10px] font-black w-6 h-6 rounded-full shadow-lg animate-bounce z-10 border-2 border-white">
                              <Bell className="w-3 h-3 mr-0.5"/>
                              {unreadCounts[a._id]}
                            </div>)}
                          {a.status !== "cancelled" && a.status !== "completed" && (<Button variant="ghost" size="sm" className="rounded-[14px] text-[10px] font-black uppercase text-rose-500 hover:bg-rose-50" onClick={() => cancelBooking(a._id)} disabled={loading}>
                              Cancel
                            </Button>)}
                          {a.status === "confirmed" && (<Button className={`rounded-[16px] px-8 py-6 font-black uppercase text-[10px] tracking-widest transition-all ${activeChatAppointmentId === a._id ? 'bg-[#1E3A8A] text-white shadow-lg' : 'bg-white text-[#1E3A8A] border border-[#1E3A8A] hover:bg-slate-50'}`} onClick={() => openChat(a._id)}>
                      ...
                      {/* Inline Chat Panel */}
                      {activeChatAppointmentId === a._id && (<div className="mt-8 rounded-[20px] border border-slate-200 bg-white p-6 shadow-xl animate-in fade-in zoom-in-95">
                          ...
                          <div className="max-h-[300px] overflow-y-auto space-y-4 pr-3 flex flex-col custom-scrollbar">
                            {messages.length === 0 ? (<div className="text-[10px] font-black uppercase text-slate-300 text-center py-12 italic tracking-widest">No communication history.</div>) : (messages.map((m) => {
                        ...
                        return (<div key={m._id} className={`flex ${isMe ? "justify-end" : "justify-start"}`}>
                          ...
                          <div className="mt-8 flex gap-3">
                            <Input value={chatText} onChange={(e) => setChatText(e.target.value)} onKeyDown={handleChatKeyDown} placeholder="Transmit message..." className="rounded-[16px] bg-slate-50 border-slate-200 py-7 px-6 text-sm font-bold placeholder:text-slate-400 outline-none focus:border-accent"/>
        ...
        // HISTORY VIEW
        historyAppointments.length === 0 ? (<div className="text-[10px] font-black uppercase tracking-widest text-slate-500 py-16 text-center opacity-40 italic">Archive empty. No historical encounters.</div>) : (<div className="space-y-4">
                  {historyAppointments.map((a) => (<Card key={a._id} className={`rounded-[24px] border ${selectedHistoryIds.includes(a._id) ? 'border-rose-400 bg-rose-50/20' : 'border-slate-100 bg-slate-50/30'} shadow-sm hover:shadow-md transition-all cursor-pointer overflow-hidden`} onClick={() => setExpandedHistoryId(expandedHistoryId === a._id ? null : a._id)}>
                      ...
                      {expandedHistoryId === a._id && (<div className="p-8 pt-4 border-t border-slate-100 bg-white animate-in slide-in-from-top-4 duration-500">
                           ...
                           {a.status === 'completed' && (<>
                               ...
                               <div className="pt-6 mt-6 flex justify-end gap-3">
                                  ...
                                  <Button className="rounded-[14px] font-black uppercase text-[10px] tracking-widest bg-[#1E3A8A] text-white shadow-md hover:bg-[#2563EB]" onClick={(e) => { e.stopPropagation(); requestChat(a.doctorId._id); }} disabled={loading}>
                                    Request to Chat
                                  </Button>
                               </div>
                             </>)}
