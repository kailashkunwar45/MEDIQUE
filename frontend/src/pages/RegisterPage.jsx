          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {["PATIENT", "DOCTOR", "HOSPITAL_ADMIN"].map((role) => (<button key={role} type="button" onClick={() => setFormData({ ...formData, role })} className={`px-3 py-2 text-xs font-bold rounded-xl border transition-all ${formData.role === role
            ...
            <div className="space-y-2">
              ...
              <Input id="name" placeholder="John Doe" className="rounded-xl bg-muted/30" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required/>
            </div>
            <div className="space-y-2">
            ...
            {(formData.role === "DOCTOR" || formData.role === "HOSPITAL_ADMIN") && (<div className="space-y-2">
                <Label htmlFor="hospitalId">Hospital Id (optional)</Label>
                <Input id="hospitalId" placeholder="Mongo ObjectId (leave empty to auto-create)" className="rounded-xl bg-muted/30 font-mono" value={formData.hospitalId} onChange={(e) => setFormData({ ...formData, hospitalId: e.target.value })}/>
              </div>)}
          </CardContent>
