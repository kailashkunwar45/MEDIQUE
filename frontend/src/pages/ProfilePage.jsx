...
function RoleBadge({ role }) {
    ...
    return (<span className={`text-xs font-semibold px-3 py-1 rounded-full border ${map[role] || "bg-muted text-muted-foreground border-muted"}`}>
      {labels[role] || role}
    </span>);
}
function getSession() {
...
function getBackHref(role) {
    const map = { patient: "/patient", doctor: "/doctor", hospital_admin: "/admin", super_admin: "/superadmin" };
    return map[role] || "/";
}
export default function ProfilePage() {
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="max-w-2xl mx-auto px-8 py-8 space-y-4">
          ...
          <CardContent className="space-y-4">
            ...
            {role === "hospital_admin" && (<>
                ...
                <div className="space-y-1">
                  ...
                  <textarea value={services} onChange={(e) => setServices(e.target.value)} placeholder="Cardiology, Emergency, etc." rows={4} className="w-full rounded-xl bg-muted/30 border border-muted px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/50"/>
                </div>
              </>)}
            ...
            {role === "doctor" && (<>
                ...
                <div className="space-y-1">
                  ...
                  <textarea value={previousWork} onChange={(e) => setPreviousWork(e.target.value)} placeholder="Where have you worked before?" rows={4} className="w-full rounded-xl bg-muted/30 border border-muted px-3 py-2 text-sm resize-none focus:outline-none focus:ring-2 focus:ring-primary/50"/>
                </div>
              </>)}
        ...
        {role === "doctor" && (<Card className="rounded-2xl border-muted shadow-lg">
            <CardHeader className="pb-2">
        ...
        {role === "patient" && (<Card className="rounded-2xl border-muted shadow-lg">
            <CardHeader className="pb-2">
