...
function TierBadge({ tier }) {
    ...
    return (<span className={`text-xs font-semibold px-2 py-0.5 rounded-full capitalize ${map[tier] || map.free}`}>
      {tier}
    </span>);
}
    ...
    useEffect(() => {
        ...
        Promise.all([
            ...
            if (Array.isArray(hospitalsData))
                ...
                setError(hospitalsData?.message || "Failed to load hospitals");
    ...
    return (<div className="min-h-screen bg-background">
      ...
      <div className="w-full bg-gradient-to-br from-emerald-500/10 via-primary/5 to-background border-b border-muted px-8 py-10">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="mt-6 max-w-md">
            <Input type="text" placeholder={activeTab === "hospitals" ? "Search hospitals by name or location..." : "Search doctors by name or specialization..."} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="rounded-xl border-muted bg-background shadow-sm"/>
          </div>
          
          <div className="mt-6 flex gap-4 border-b border-muted/30 pb-2">
            <button className={`pb-2 text-sm font-bold uppercase tracking-widest transition-all ${activeTab === "hospitals" ? "text-primary border-b-2 border-primary" : "text-muted-foreground hover:text-foreground"}`} onClick={() => setActiveTab("hospitals")}>
              Hospitals ({filteredHospitals.length})
            </button>
            <button className={`pb-2 text-sm font-bold uppercase tracking-widest transition-all ${activeTab === "doctors" ? "text-primary border-b-2 border-primary" : "text-muted-foreground hover:text-foreground"}`} onClick={() => setActiveTab("doctors")}>
      ...
      <div className="max-w-5xl mx-auto px-8 py-8">
        ...
        {!loading && activeTab === "hospitals" && (<>
            {filteredHospitals.length === 0 && !error && (<div className="text-center py-16 text-muted-foreground italic">No hospitals found matching "{searchQuery}".</div>)}
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filteredHospitals.map((h) => (<Card key={h._id} className="rounded-2xl border-muted shadow-lg hover:shadow-xl hover:border-primary/30 transition-all duration-200 group">
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    {h.contactPhone && (<div className="flex items-center gap-2 text-muted-foreground">
                        <span>📞</span>
        ...
        {!loading && activeTab === "doctors" && (<>
            {filteredDoctors.length === 0 && !error && (<div className="text-center py-16 text-muted-foreground italic">No specialists found matching "{searchQuery}".</div>)}
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {filteredDoctors.map((d) => (<Card key={d._id} className="rounded-2xl border-muted shadow-lg hover:shadow-xl hover:border-primary/30 transition-all duration-200 group">
                  <CardHeader className="pb-2">
                    <div className="flex items-start justify-between gap-2">
                      <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-lg font-black text-white shrink-0">
                        {d.name.charAt(0)}
                      </div>
                      {d.specialization && (<span className="text-[10px] font-black uppercase tracking-widest text-primary border border-primary/20 bg-primary/10 px-2 py-0.5 rounded-full">
                          {d.specialization}
                        </span>)}
                    </div>
                    <CardTitle className="text-base mt-2 group-hover:text-primary transition-colors">
