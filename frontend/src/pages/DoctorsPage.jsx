...
function Stars({ value }) {
    return (<span className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (<span key={s} className={s <= Math.round(value) ? "text-amber-400" : "text-muted-foreground/30"}>
          ★
        </span>))}
    </span>);
}
...
function DoctorProfileContent() {
    ...
    const loadProfile = () => {
        ...
        Promise.all([profileFetch, apptFetch])
            ...
            if (d?.doctor)
                ...
                setError(d?.message || "Doctor not found");
            if (Array.isArray(appts)) {
                const hasCompleted = appts.some((a) => String(a.doctorId?._id || a.doctorId) === String(id) && a.status === 'completed');
    ...
    const initials = doctor.name.split(" ").map((w) => w[0]).slice(0, 2).join("").toUpperCase();
    return (<div className="min-h-screen bg-background">
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            ...
            <div className="text-center sm:text-left flex-1">
              <h1 className="text-3xl font-bold tracking-tight">{doctor.name}</h1>
               {doctor.specialization && (<div className="mt-1 inline-flex items-center gap-2">
                   <div className="bg-primary/10 text-primary text-sm font-semibold px-3 py-0.5 rounded-full border border-primary/20">
                     {doctor.specialization}
                   </div>
                   {doctor.appointmentFee && (<div className="bg-emerald-500/10 text-emerald-600 text-sm font-semibold px-3 py-0.5 rounded-full border border-emerald-500/20">
            ...
            {session?.role === "patient" && (<Button className="rounded-xl shadow-lg shadow-primary/20" onClick={() => setShowBooking(true)}>
                Book Appointment
              </Button>)}
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-8 py-8 grid gap-6 md:grid-cols-3">
        ...
        <div className="space-y-6 md:col-span-1">
          <Card className="rounded-2xl border-muted shadow-lg">
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              {doctor.phone && (<div className="flex items-center gap-2">
                  <span>📞</span>
                  <a href={`tel:${doctor.phone}`} className="hover:underline">{doctor.phone}</a>
                </div>)}
              {doctor.hospitalId?.contactPhone && (<div className="flex items-center gap-2">
                  <span>🏥</span>
          ...
          {doctor.bio && (<Card className="rounded-2xl border-muted shadow-lg">
              <CardHeader className="pb-2">
        ...
        <div className="space-y-6 md:col-span-2">
              ...
              <CardContent className="space-y-3">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex gap-1">
                    {[1, 2, 3, 4, 5].map((s) => (<button key={s} type="button" onClick={() => setRating(s)} className={`text-2xl transition-transform hover:scale-110 ${s <= rating ? "text-amber-400" : "text-muted-foreground/30"}`}>
                        ★
                      </button>))}
          ...
          <Card className="rounded-2xl border-muted shadow-lg">
            ...
            <CardContent>
              {reviews.length === 0 ? (<div className="text-sm text-muted-foreground">No reviews yet.</div>) : (<div className="space-y-4">
      ...
      {showBooking && data?.doctor.hospitalId && (<BookingModal preHospitalId={data.doctor.hospitalId._id} preHospitalName={data.doctor.hospitalId.name} preDoctorId={data.doctor._id} preDoctorName={data.doctor.name} preSpecialization={data.doctor.specialization} onClose={() => setShowBooking(false)}/>)}
    </div>);
}
