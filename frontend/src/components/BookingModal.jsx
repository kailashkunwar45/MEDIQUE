...
export default function BookingModal({ preHospitalId, preHospitalName, preDoctorId, preDoctorName, preSpecialization, onClose, }) {
    const session = getSession();
    // Step: "spec" | "doctor" | "confirm"
    const [step, setStep] = useState(preDoctorId ? "confirm" : preHospitalId ? "spec" : "spec");
    ...
    useEffect(() => {
        ...
        fetch(`${API}/api/hospitals/detail?id=${preHospitalId}/doctors`)
            ...
            if (!Array.isArray(docs))
                return;
            const specs = Array.from(new Set(docs.map(d => d.specialization).filter(Boolean)));
    ...
    useEffect(() => {
        if (!preHospitalId || !selectedSpec || preDoctorId)
            return;
        fetch(`${API}/api/hospitals/detail?id=${preHospitalId}/doctors`)
            .then(r => r.json())
            .then((docs) => {
    ...
    if (!session?.accessToken) {
        return (<ModalShell onClose={onClose} title="Book Appointment">
    ...
    return (<ModalShell onClose={onClose} title="Book Appointment">
      ...
      {step === "spec" && !preDoctorId && (<div className="space-y-4">
          <div>
          ...
          {specializations.length === 0 ? (<p className="text-xs text-muted-foreground italic">Loading available specializations...</p>) : (<div className="grid grid-cols-2 gap-2">
              {specializations.map(s => (<button key={s} onClick={() => { setSelectedSpec(s); setStep("doctor"); }} className="text-xs font-semibold px-3 py-3 rounded-xl border border-muted bg-muted/10 hover:border-primary/40 hover:bg-primary/5 text-left transition-all">
                  {s}
                </button>))}
            </div>)}
      ...
      {step === "doctor" && (<div className="space-y-4">
          <div>
          ...
          {doctors.length === 0 ? (<p className="text-xs text-muted-foreground italic">No doctors found for this specialization.</p>) : (<div className="space-y-2">
              {doctors.map(d => (<button key={d._id} onClick={() => { setSelectedDoctorId(d._id); setSelectedDoctorName(d.name); setStep("confirm"); }} className={`w-full text-left px-4 py-3 rounded-xl border transition-all text-sm font-medium ${selectedDoctorId === d._id ? "border-primary bg-primary/10 text-primary" : "border-muted bg-muted/10 hover:border-primary/30"}`}>
                  {d.name}
      ...
      {step === "confirm" && (<div className="space-y-4">
          {!preDoctorId && (<button onClick={() => setStep("doctor")} className="text-xs text-primary hover:underline">← Change doctor</button>)}
          ...
          <div className="rounded-xl border border-muted bg-muted/10 p-4 text-sm space-y-1">
            ...
            {doctors.find(d => d._id === selectedDoctorId)?.appointmentFee && (<div className="pt-1 text-[#1E3A8A] font-black uppercase text-[10px] tracking-widest">
          ...
          <div className="space-y-1">
            ...
            <input type="date" value={date} min={new Date().toISOString().slice(0, 10)} onChange={e => setDate(e.target.value)} className="w-full rounded-xl bg-background border border-muted px-4 py-3 text-sm outline-none focus:border-primary [color-scheme:dark]"/>
          </div>

          <div className="space-y-1">
            <Label className="text-xs font-black uppercase text-muted-foreground tracking-widest">Payment</Label>
            <div className="grid grid-cols-2 gap-2">
              {["online", "pay_later"].map(m => (<button key={m} onClick={() => setPaymentMethod(m)} className={`py-3 text-xs font-bold rounded-xl border transition-all ${paymentMethod === m ? "bg-primary text-primary-foreground border-primary" : "bg-muted/10 border-muted hover:border-primary/30"}`}>
          ...
          {info ? (<Button className="w-full rounded-xl" onClick={onClose}>Go to Dashboard</Button>) : (<Button className="w-full rounded-xl" onClick={bookAppointment} disabled={loading}>
...
function ModalShell({ children, onClose, title }) {
    return (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" onClick={e => { if (e.target === e.currentTarget)
        onClose(); }}>
